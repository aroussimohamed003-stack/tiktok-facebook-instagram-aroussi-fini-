...
if (!$con) {
    die("<h1>Database Connection Error</h1><p>Cannot connect to database. Please check config.php</p>");
}
...
$required_tables = ['market_categories', 'market_subcategories', 'market_products'];
foreach ($required_tables as $table) {
    $check = $con->query("SHOW TABLES LIKE '$table'");
    if (!$check || $check->num_rows == 0) {
        die("<h1>Setup Required</h1><p>Marketplace tables are not set up. Please run <a href='setup_marketplace.php'>setup_marketplace.php</a> first.</p>");
    }
}

// Get Categories and Subcategories with error handling
$categories = [];
$subcategories = [];

$cats_query = $con->query("SELECT * FROM market_categories ORDER BY id ASC");
if (!$cats_query) {
    die("<h1>Database Error</h1><p>Error fetching categories: " . $con->error . "</p>");
}
...
<body class="market-page">
    ...
    <div class="market-header">
        ...
        <div style="display: flex; align-items: center; gap: 15px;">
            <select id="currencyViewer" class="form-control" style="width:auto; height:32px; padding:0 10px; font-size:12px; background:rgba(255,255,255,0.1); border:none; color:white; border-radius:15px;" onchange="loadProducts()">
            ...
            <form id="addForm" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <div style="display:flex; gap:5px;">
                        <input type="number" name="price" class="form-control" required placeholder="0.00" style="flex:1;">
    ...
    <script>
        ...
        function loadProducts() {
            ...
            $('#productContainer').html('<div style="grid-column: 1/-1; text-align:center; padding:40px; color:#666;"><i class="fas fa-spinner fa-spin fa-2x"></i><br><br>Loading products...</div>');
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    try {
                        ...
                        console.error('JSON Parse Error:', e, res);
                        $('#productContainer').html('<div style="grid-column: 1/-1; text-align:center; padding:20px; color:#f44336;"><i class="fas fa-exclamation-triangle"></i><br>Server Error: Invalid response format<br><small>Check console for details</small></div>');
                        return;
                    }
                        ...
                        if(data.products.length > 0) {
                            data.products.forEach(p => {
                                ...
                                if (viewCurr === 'ORIGINAL' || viewCurr === pCurr) {
                                    ...
                                    priceDisplay = symbol + pPrice.toFixed(2);
                                } else {
                                    // Convert
                            ...
                            html = '<div style="grid-column: 1/-1; text-align:center; padding:40px; color:#666;"><i class="fas fa-box-open fa-3x" style="opacity:0.3;"></i><br><br>No products found in this category.</div>';
                        }
                        $('#productContainer').html(html);
                    } else {
                        $('#productContainer').html('<div style="grid-column: 1/-1; text-align:center; padding:20px; color:#f44336;"><i class="fas fa-exclamation-triangle"></i><br>' + (data.message || 'Error loading products') + '</div>');
                    }
                },
                error: function(xhr, status, error) {
                    console.error('AJAX Error:', status, error, xhr.responseText);
                    $('#productContainer').html('<div style="grid-column: 1/-1; text-align:center; padding:20px; color:#f44336;"><i class="fas fa-wifi"></i><br>Network/Server Error<br><small>Please check your connection and try again</small></div>');
                }
            });
        }
