    ...
    if ($_GET['action'] == 'send_message' && isset($_POST['receiver_id'])) {
        ...
        if ($message != '') {
            ...
            if ($stmt->execute()) {
                ...
                echo json_encode(['success' => false, 'error' => 'db_error: ' . $con->error]);
            }
        } else {
...
// Only show users where there is at least one message with type='market' between us.
$users = [];
$sql_users = "
...
<body>
    ...
    <script>
        ...
        function fetchMessages() {
            ...
            fetch(`market_messages.php?action=fetch_chat&user_id=${currentUserId}`)
                .then(r => r.json())
                .then(data => {
                    if(data.success) {
                        ...
                        data.messages.forEach(msg => {
                            ...
                            html += `
                                <div class="message-row ${msg.type}">
                                    <div class="message-bubble" style="${msg.product_id_ref ? 'border:1px solid gold;' : ''}">
                                        ${(msg.product_id_ref && msg.type=='sent') ? `<div style="font-size:10px; color:#aaa; margin-bottom:5px;">Regarding: ${msg.product_title}</div>` : '' }
                                        ${content}
        ...
        let pendingProductId = null; // Store product ID if passed in URL
        ...
        function sendMessage() {
            ...
            // append product link if first message of session for this product
            
            // Actually, we pass product_id to the endpoint now
            ...
            if(pendingProductId) {
                ...
                // Only attach once? Or every time? 
                // Let's attach it. But usually we only need it once to establish context.
                // But the schema allows every message to reference a product.
                // Let's keep sending it if the user is in "Product Context" mode.
                // But simplified: reset it after first send?
                // The URL param "product_id" implies we are starting a chat ABOUT this product.
                // So the first message should have it.
            }

            fetch(`market_messages.php?action=send_message`, {
                method: 'POST',
                body: formData
            })
            .then(r => r.json())
            .then(data => {
                if(data.success) {
                    fetchMessages();
                    pendingProductId = null; // Clear after sending one message with context? Or keep it?
                    // User might switch topic. Let's clear it to avoid tagging every subsequent 'ok' with the product.
                } else {
        ...
        function deleteConversation() {
            ...
            if(confirm('Are you sure you want to delete this conversation?')) {
                ...
                fetch('market_messages.php?action=delete_conversation', { method: 'POST', body: fd })
                .then(r => r.json())
                .then(data => {
                    if(data.success) {
        ...
        function deleteMessage(msgId) {
            ...
            if(confirm('Delete this message?')) {
                ...
                fetch('market_messages.php?action=delete_message', { method: 'POST', body: fd })
                .then(r => r.json())
                .then(data => {
                    if(data.success) {
                        fetchMessages();
        ...
        const autoProductId = urlParams.get('product_id');
        
        if (autoProductId) pendingProductId = autoProductId;
        ...
        if (autoUserId) {
             ...
             if (userItem) {
                 ...
                 // User might not be in the list if this is a NEW market chat.
                 // We need to fetch basic user info to open the chat window properly.
                 // Let's do a quick fetch to get user name/pic
                 // This is a "phantom" chat until a message is sent
                // Fetch user info from an endpoint (we repurpose message.php action or just use a new one)
                // We'll just manually set the header and valid currentUserId
                // We can fetch via profile API or assume global variable available? No.
                // Let's fetch public profile info?
                // Hack: We don't have a simple "get_user_info" endpoint ready.
                // We can use the existing `message.php?action=fetch_chat` on a new user, it returns empty messages but we need name/pic.
                // But fetch_chat returns {messages:[], receiver_pic: ...}. It doesn't return name.
                // Let's just USE placeholders, and maybe the first fetchMessages will help? No name.
                // Let's Add a helper to get user info in PHP section?
                // Or just create a temporary UI element.
                openChat(autoUserId, 'Seller', 'uploads/profile.jpg');
                // The header name will be 'Seller'. It's okay-ish. 
             }
        }
    </script>
</body>
