    ...
    if (move_uploaded_file($_FILES['chunk']['tmp_name'], $chunkFile)) {
        ...
        if (count($uploadedChunks) == $totalChunks) {
            ...
            if ($out = fopen($finalPath, "wb")) {
                ...
                // Use existing $con from config.php
            ...
            echo "Uploaded chunk " . $chunkIndex;
        }
    } else {
        http_response_code(500);
                ...
                <div class="relative group">
                    <label for="fileInput" class="block w-full cursor-pointer">
                        <div class="border-2 border-dashed border-slate-200 rounded-3xl p-10 text-center transition-all group-hover:border-blue-500 group-hover:bg-blue-50/30">
                            ...
                            <h4 class="text-sm font-black text-slate-700 mb-1">Select Vertical Video</h4>
                            <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">MP4, WEBM (Max 50MB)</p>
    ...
    fileInput.addEventListener('change', function() {
        ...
        video.onloadedmetadata = function() {
            ...
            if (video.videoWidth > video.videoHeight) {
                showMessage('Vertical videos only, please.', 'bg-rose-50 text-rose-600');
                fileInput.value = ''; 
                fileNameDisplay.classList.add('hidden');
            } else {
        ...
        video.onerror = function() {
            showMessage('Invalid video format.', 'bg-rose-50 text-rose-600');
            fileInput.value = '';
            fileNameDisplay.classList.add('hidden');
        };
