...
<div class="bg-white rounded-[1.5rem] md:rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden h-[calc(100dvh-100px)] md:h-[calc(100vh-140px)] flex flex-col md:flex-row relative">
    ...
    <div class="flex-1 flex flex-col h-full bg-slate-50/50 relative">
        ...
        <!-- Active Chat -->
        <div id="activeChat" class="hidden flex-col h-full transition-opacity duration-300">
            <!-- Header -->
            ...
            <!-- Messages Stream -->
            <div class="flex-1 overflow-y-auto p-8 flex flex-col gap-6 custom-scrollbar" id="messagesContainer" style="overflow-anchor: auto;">
                <!-- Injected via AJAX -->
            </div>

            <!-- Input Bar -->
            <div class="p-3 md:p-6 bg-white border-t border-slate-50">
                <div class="bg-slate-50 p-1.5 md:p-2 rounded-[2rem] flex items-center gap-1 md:gap-2 shadow-inner group-focus-within:bg-white group-focus-within:ring-2 ring-blue-500/10 transition-all">
                    <div class="flex gap-0.5 md:gap-1 pl-1 md:pl-2">
                        <button onclick="triggerFile('image')" class="w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center text-slate-400 hover:text-blue-500 hover:bg-blue-50 transition-all rotate-12"><i class="fas fa-image text-xs md:text-base"></i></button>
                        <button onclick="triggerFile('video')" class="w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center text-slate-400 hover:text-indigo-500 hover:bg-indigo-50 transition-all -rotate-12"><i class="fas fa-clapperboard text-xs md:text-base"></i></button>
                    </div>
                    
                    <input type="text" id="msgInput" placeholder="Message discovery agent..." class="flex-1 bg-transparent border-none focus:ring-0 text-sm font-medium text-slate-700 px-2 md:px-4 min-w-0">
                    
                    <button id="voiceBtn" onclick="toggleRecording()" class="w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center text-slate-400 hover:text-rose-500 hover:bg-rose-50 transition-all"><i class="fas fa-microphone text-xs md:text-base"></i></button>
                    
                    <div id="recordingStatus" class="hidden px-2 md:px-4 items-center gap-1 md:gap-2">
                        <span class="w-1.5 h-1.5 md:w-2 md:h-2 bg-rose-500 rounded-full animate-ping"></span>
                        <span id="recTimer" class="text-[9px] md:text-[10px] font-black text-rose-500">00:00</span>
                    </div>

                    <button onclick="sendMessage()" class="w-10 h-10 md:w-12 md:h-12 rounded-full bg-blue-600 text-white flex items-center justify-center shadow-lg shadow-blue-500/30 hover:bg-blue-700 hover:scale-110 active:scale-95 transition-all flex-shrink-0">
                        <i class="fas fa-paper-plane text-xs md:text-base"></i>
                    </button>
                </div>
                
                <div id="uploadProgress" class="hidden mt-2 h-1 w-full bg-slate-100 rounded-full overflow-hidden">
...
.message-bubble { @apply px-5 py-3 rounded-[1.5rem] max-w-[85%] text-[13px] font-medium shadow-sm relative overflow-hidden transition-all; }
.received .message-bubble { @apply bg-white text-slate-800 rounded-bl-none border border-slate-200 shadow-sm; }
.sent .message-bubble { @apply bg-blue-600 text-white rounded-br-none shadow-md shadow-blue-200 bg-gradient-to-br from-blue-600 to-indigo-600; }
.sent .message-bubble .message-time { @apply text-blue-100; }
.received .message-bubble .message-time { @apply text-slate-400; }
.message-time { @apply text-[9px] font-black uppercase tracking-widest mt-1 block opacity-80; }
...
<script>
    ...
    function openChat(uid, name, pic) {
        ...
        $('#activeChat').removeClass('hidden').addClass('flex opacity-100');
    ...
    function fetchMessages(force = false) {
        ...
        $.get(`message.php?action=fetch_chat&user_id=${currentUserId}`, function(res) {
            if(res.success) {
                ...
                if(isBlockedByMe) $('#msgInput').attr('placeholder', 'You blocked this agent').prop('disabled', false);
                else if(amIBlocked) $('#msgInput').attr('placeholder', 'Agent restricted your access').prop('disabled', false);
                else $('#msgInput').prop('disabled', false).attr('placeholder', 'Message discovery agent...');
                
                const container = $('#messagesContainer');
                const el = container[0];
                
                // Avoid refreshing if audio is playing
                const isPlayingAudio = container.find('audio').filter((_, el) => !el.paused).length > 0;
                if (isPlayingAudio) return;

                // Detect activity and current scroll position
                const wasAtBottom = (el.scrollHeight - el.scrollTop - el.clientHeight) <= 150;
                
                // CRITICAL FIX: If user is reading old messages (not at bottom), 
                // skip the update to prevent "actualiser" jumps. 
                // Only update if forced (e.g. sending a message) or at the bottom.
                if (container.html() !== html) {
                    if (!force && !wasAtBottom && container.children().length > 0) return;
    ...
    $('#messagesContainer').on('scroll', function() {
        ...
        // If user scrolls back to bottom, trigger a refresh to show any pending messages
        const el = this;
    ...
    $('#messagesContainer').on('touchstart mousedown', function() {
        isTouching = true;
    });
    ...
    function sendMessage() {
        ...
        $.post('message.php?action=send_message', { receiver_id: currentUserId, message: txt }, function(res) {
            if(res.success) { $('#msgInput').val(''); fetchMessages(true); }
            else if(res.error === 'blocked') alert('لا يمكنك الإرسال، هذا الحساب محظور (Blocked)');
            else alert('فشل الإرسال (Send failed)');
        });
    }

    function deleteMsg(id) {
        if(confirm('Erase this data packet?')) {
            $.post('message.php?action=delete_message', { message_id: id }, () => fetchMessages(true));
        }
    }
    ...
    function toggleBlock() {
        ...
        if(confirm(isBlockedByMe ? 'Establish reconnection?' : 'Restict this frequency?')) {
            $.post(`message.php?action=${action}`, { user_id: currentUserId }, () => fetchMessages(true));
        }
    }
    ...
    function startUpload(file) {
        ...
        $.ajax({
            ...
            success: (res) => {
                ...
                if(res.success) fetchMessages(true);
            }
        });
    }
