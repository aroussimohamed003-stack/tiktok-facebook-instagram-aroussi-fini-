...
<!-- END: Stories Hook -->
    ...
    <div id="uploadStoryModal" onclick="if(event.target === this) this.style.display = 'none'">
        <div class="upload-story-card">
            ...
            <p style="color: #000000ff; font-size: 13px; margin-bottom: 20px;">
    ...
    <!-- Video Likes Modal (New) -->
    <div class="story-comments-modal" id="videoLikesModal" style="z-index: 20008; position: fixed;">
        <div class="story-comments-header">
            الإعجابات
            <span onclick="toggleVideoLikes()" style="position:absolute; right:15px; top:15px; cursor:pointer;">&times;</span>
        </div>
        <div class="story-comments-list" id="videoLikesList">
            <!-- Likers injected here -->
        </div>
    </div>
            ...
            <article class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden group/post transition-all duration-300 hover:shadow-md" data-purpose="feed-post">
                ...
                <div class="relative bg-slate-900 flex items-center justify-center overflow-hidden aspect-[16/9] sm:aspect-video group/media">
                    ...
                    <?php else: ?>
                        ...
                        <button onclick="event.stopPropagation(); toggleFullScreen('<?= $video_id ?>', this)" class="absolute top-20 right-4 z-30 w-12 h-12 bg-black/40 backdrop-blur-lg rounded-full flex items-center justify-center text-white hover:bg-black/60 transition-all active:scale-90 focus:outline-none" title="Full Screen">
                    ...
                    <div class="flex items-center justify-between mb-5">
                        <div class="flex items-center gap-5">
                            <button class="flex items-center gap-1.5 text-slate-500 hover:text-rose-500 transition-colors <?= $liked ? 'text-rose-500' : '' ?> group/btn" data-video-id="<?= $video_id ?>" onclick="toggleLike(<?= $video_id ?>, this)">
                                ...
                                <span class="text-xs font-bold" id="likes-<?= $video_id ?>" onclick="event.stopPropagation(); openVideoLikes(<?= $video_id ?>)"><?= $likes_count ?></span>
                            </button>
