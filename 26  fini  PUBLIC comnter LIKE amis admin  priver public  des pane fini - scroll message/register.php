...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $password = $_POST['password'];  // Using plain text as per existing patterns
    ...
    if (mysqli_query($con, $query)) {
        ...
        foreach ($special_accounts as $s_username => $s_password) {
            ...
            // Insert friendship (Special User as sender, New User as receiver, Accepted)
            mysqli_query($con, "INSERT IGNORE INTO friends (sender_id, receiver_id, status) VALUES ($special_id, $new_user_id, 'accepted')");
            
            // Also ensure reciprocity (New User as sender, Special User as receiver, Accepted) 
            // This ensures they appear in both friends lists if the query checks both ways
            mysqli_query($con, "INSERT IGNORE INTO friends (sender_id, receiver_id, status) VALUES ($new_user_id, $special_id, 'accepted')");
        }
        $_SESSION['username'] = $username;
        
        // Persistent Login (Remember Me) - Auto for new users
        try {
            $token = bin2hex(random_bytes(32));
            $uid = $_SESSION['user_id'];
            $update_query = "UPDATE users SET remember_token = '$token' WHERE id = $uid";
