...
if (isset($_GET['video_id']) || isset($_GET['post_id'])) {
    ...
    if (mysqli_num_rows($likes_query) > 0) {
        ...
        $text_color = ($theme == 'light') ? '#1e293b' : 'white';
        $border_color = ($theme == 'light') ? 'rgba(0,0,0,0.05)' : 'rgba(255,255,255,0.1)';
        ...
        while ($liker = mysqli_fetch_assoc($likes_query)) {
            ...
            echo '<li style="display: flex; align-items: center; gap: 10px; padding: 10px; border-bottom: 1px solid '.$border_color.'; list-style: none;">';
            echo '<a href="profile.php?id=' . $liker['user_id'] . '" style="display: flex; align-items: center; gap: 10px; text-decoration: none; color: '.$text_color.'; width: 100%;">';
            ...
            echo '<span style="font-weight: 600; color: '.$text_color.';">' . htmlspecialchars($liker['username']) . '</span>';
            echo '</a>';
            echo '</li>';
        }
    } else {
