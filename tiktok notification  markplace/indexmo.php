...
<script src="https://cdn.jsdelivr.net/npm/nsfwjs@2.4.1/dist/nsfwjs.min.js"></script>
    ...
    <div class="story-comments-modal" id="videoCommentsModal" style="z-index: 6000;">
        ...
        <div class="story-comments-footer">
            <input type="text" id="videoCommentInput" class="story-comment-input" placeholder="Add a comment...">
...
function toggleVideoComments() {
    $('#videoCommentsModal').slideToggle();
}

function openVideoComments(videoId) {
    currentVideoIdForComments = videoId;
    $('#videoCommentsModal').slideDown();
    loadVideoComments(videoId);
}

function loadVideoComments(videoId) {
    $('#videoCommentsList').html('<div class=\"text-center mt-3\"><div class=\"spinner-border text-light\"></div></div>');
    ...
    $.get('indexmo.php?action=get_video_comments&video_id=' + videoId, function(data) {
        ...
        if(data.success && data.comments.length > 0) {
            data.comments.forEach(c => {
                let deleteHtml = '';
                if(data.current_user_id == c.user_id) {
                    deleteHtml = `<span class=\"story-comment-delete\" onclick=\"deleteVideoComment(\${c.id})\">Delete</span>`;
                }
                ...
                <div class=\"story-comment-item\">
                    ...
                    <div class=\"story-comment-content\">
                        ...
                        <div class=\"story-comment-text\">\${c.comment}</div>
                    </div>
                    \${deleteHtml}
                </div>`;
            });
        } else {
            html = '<div class=\"text-center text-muted mt-3\">No comments yet.</div>';
        }
...
function postVideoComment() {
    ...
    let txt = $('#videoCommentInput').val().trim();
    if(!txt) return;
    
    $('#videoCommentInput').val(''); // Clear immediately
