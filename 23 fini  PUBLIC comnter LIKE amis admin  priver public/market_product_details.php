...
<body class="market-page">
    ...
    <div class="market-header">
        ...
        <div style="display: flex; align-items: center; gap: 10px;">
            <select id="currencyViewer" class="form-control" style="width:auto; height:32px; padding:0 10px; font-size:12px; background:rgba(255,255,255,0.1); border:none; color:white; border-radius:15px;" onchange="updatePriceDisplay()">
                <option value="ORIGINAL">Original</option>
                <option value="USD">USD ($)</option>
                <option value="TND">TND (د.ت)</option>
                <option value="EUR">EUR (€)</option>
            </select>
            <i class="fas fa-share-alt fa-lg" style="color:white; cursor:pointer;"></i>
        </div>
    </div>
    ...
    <div class="details-container">
        ...
        <h2 id="priceDisplay" style="color: var(--market-primary);">
            <?php 
                $pCurr = $product['currency'] ?? 'USD';
                $symbol = $pCurr === 'USD' ? '$' : ($pCurr === 'TND' ? 'د.ت' : '€');
                echo $symbol . htmlspecialchars($product['price']);
            ?>
        </h2>
    ...
    <script>
        ...
        function updatePriceDisplay() {
            ...
            let pCurr = '<?= $product['currency'] ?? 'USD' ?>';
