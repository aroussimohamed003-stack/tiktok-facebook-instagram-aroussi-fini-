<?php
session_start();
include("includes/config.php"); // Use central config
$conn = $con;
    ...
    if (move_uploaded_file($_FILES['chunk']['tmp_name'], $chunkFile)) {
        ...
        if (count($uploadedChunks) == $totalChunks) {
            ...
            if ($out = fopen($_SERVER['DOCUMENT_ROOT'] . "/" . $finalPath, "wb")) {
                ...
                fclose($out);
                $stmt = $conn->prepare("INSERT INTO videos (location, subject, title, user_id, privacy) VALUES (?, ?, ?, ?, ?)");
...
<a href="index.php"> رجوع لصفحة رئسية </a>
  ...
  <div class="container">
    ...
    <select id="privacy" style="width: 100%; margin: 10px 0; padding: 10px; border-radius: 5px; border: 1px solid #ccc; background: white;">
        <option value="public">الكل (Public)</option>
        <option value="private">خاص بي (Private)</option>
    </select>
