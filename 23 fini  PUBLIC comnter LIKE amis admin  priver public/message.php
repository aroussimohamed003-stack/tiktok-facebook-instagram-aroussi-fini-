...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] == 'search_users' && isset($_GET['query'])) {
        ...
        $filter = isset($_GET['filter']) ? $_GET['filter'] : 'all'; // 'all' or 'friends'
        ...
        while($row = $res->fetch_assoc()) {
            $row['unread'] = $con->query("SELECT COUNT(*) as cnt FROM messages WHERE sender_id = " . $row['id'] . " AND receiver_id = $my_id AND is_read = 0")->fetch_assoc()['cnt'];
...
$users = [];
$sql_users = "SELECT id, username, profile_picture FROM users 
              WHERE id != $my_id 
              AND (
                  id IN (SELECT sender_id FROM friends WHERE receiver_id = $my_id AND status = 'accepted')
                  OR id IN (SELECT receiver_id FROM friends WHERE sender_id = $my_id AND status = 'accepted')
                  OR id IN (SELECT sender_id FROM messages WHERE receiver_id = $my_id)
                  OR id IN (SELECT receiver_id FROM messages WHERE sender_id = $my_id)
              )
              ORDER BY id DESC";
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <div class="sidebar-search">
            <div class="search-box">
                ...
                <input type="text" id="userSearchInput" placeholder="Search for people..." onkeyup="handleSearch()">
            </div>
        </div>

        <div class="filter-tabs">
            <div class="filter-tab active" data-filter="recent" onclick="setFilter('recent')">Recent</div>
            <div class="filter-tab" data-filter="all" onclick="setFilter('all')">All</div>
            <div class="filter-tab" data-filter="friends" onclick="setFilter('friends')">Friends</div>
        </div>
    ...
    <div class="chat-area">
        ...
        <div id="activeChat" style="display: none; display: flex; flex-direction: column; height: 100%; width: 100%;">
            <div class="chat-header">
                ...
                <button id="blockBtn" onclick="toggleBlock()" style="background:none; border:none; color:#ffa500; font-size: 18px; cursor: pointer; margin-right: 10px;" title="Block/Unblock User">
    ...
    <script>
        ...
        let searchQuery = '';
        const initialSidebarHtml = document.getElementById('sidebarUserList').innerHTML;
        ...
        function handleSearch() {
            ...
            if (searchQuery === '' && currentFilter === 'recent') {
                document.getElementById('sidebarUserList').innerHTML = initialSidebarHtml;
                return;
            }

            // Show loading or just fetch
            fetch(`message.php?action=search_users&query=${encodeURIComponent(searchQuery)}&filter=${currentFilter}`)
                .then(r => r.json())
                .then(data => {
                    if (data.success) {
                        const container = document.getElementById('sidebarUserList');
                        let html = '';
                        if (data.users.length === 0) {
                            html = '<div class="text-center p-3 text-muted">No users found</div>';
                        } else {
                            data.users.forEach(u => {
                                html += `
                                    <div class="user-item" onclick="openChat(${u.id}, '${u.username.replace(/'/g, "\\'")}', '${u.pic}')">
                                        <img src="${u.pic}" class="avatar" alt="Avatar" onerror="this.src='uploads/profile.jpg'">
                                        <div class="user-info">
        ...
        function toggleBlock() {
            ...
            const action = isBlockedByMe ? 'unblock_user' : 'block_user';
            const confirmMsg = isBlockedByMe ? "Unblock this user?" : "Block this user? You won't be able to send messages.";
            
            if(confirm(confirmMsg)) {
                let formData = new FormData();
                formData.append('user_id', currentUserId);
                fetch(`message.php?action=${action}`, {
                    method: 'POST',
                    body: formData
                })
                .then(r => r.json())
