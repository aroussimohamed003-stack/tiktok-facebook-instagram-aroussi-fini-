...
// Fetch user videos - only if my profile OR friends
$can_view_content = $is_my_profile || $friend_status == 'friend';
...
<?php
// Set page title
$pageTitle = $is_my_profile ? "My Profile" : htmlspecialchars($user_data['username']) . "'s Profile";
...
<div class="container mt-5">
  ...
  <div class="profile-header">
    ...
    <?php else: ?>
        <img src="<?php echo !empty($user_data['profile_picture']) ? htmlspecialchars($user_data['profile_picture']) : 'uploads/profile.jpg'; ?>" class="profile-pic mb-3" onerror="this.src='uploads/profile.jpg'">
    <?php endif; ?>
    ...
    <?php else: ?>
        <div id="friend-btn-container" class="mt-2">
            <?php if ($friend_status == 'none'): ?>
                <button class="btn btn-primary btn-sm" onclick="friendRequest(<?php echo $profile_id; ?>, 'add')">
            ...
            <?php elseif ($friend_status == 'received'): ?>
                <button class="btn btn-success btn-sm" onclick="friendRequest(<?php echo $profile_id; ?>, 'accept')">
                    <i class="fas fa-check"></i> قبول طلب الصداقة
                </button>
            <?php elseif ($friend_status == 'friend'): ?>
                <button class="btn btn-outline-primary btn-sm" onclick="if(confirm('هل تريد إلغاء الصداقة؟')) friendRequest(<?php echo $profile_id; ?>, 'unfriend')">
                    <i class="fas fa-user-friends"></i> أنتم أصدقاء
                </button>
                <a href="message.php?user_id=<?php echo $profile_id; ?>" class="btn btn-primary btn-sm">
