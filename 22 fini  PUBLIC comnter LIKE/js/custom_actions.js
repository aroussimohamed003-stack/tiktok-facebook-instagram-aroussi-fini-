        ...
        success: function (response) {
            let profileHtml = `
                <div class="profile-header">
                    ...
                    ${response.bio ? `<p class="text-muted mt-2">${response.bio}</p>` : ''}
                </div>
                <h5 class="mb-3 border-bottom pb-2" style="color:#FE2C55;">الفيديوهات</h5>
            ...
            if (response.videos && response.videos.length > 0) {
                ...
                profileHtml += '<p class="text-center text-muted w-100 p-4">لا توجد فيديوهات بعد.</p>';
            }
            profileHtml += '</div>';
