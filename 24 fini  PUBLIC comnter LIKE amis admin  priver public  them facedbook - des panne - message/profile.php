            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <?php while ($row = mysqli_fetch_assoc($fetchUserVideos)): ?>
                    <div class="relative group aspect-[3/4] rounded-2xl overflow-hidden bg-slate-900 border border-slate-100 shadow-sm">
                        <video src="<?= htmlspecialchars($row['location']) ?>" class="w-full h-full object-cover cursor-pointer video-player" data-id="<?= $row['id'] ?>" onclick="this.paused ? this.play() : this.pause()" loop muted playsinline></video>
                        
                        <!-- Overlay -->
                        <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-end p-4 pointer-events-none">
                            ...
                            <div class="flex items-center justify-between pointer-events-auto">
                                ...
                                <div class="flex gap-2">
                                    <button onclick="$(this).closest('.relative').find('video')[0].play()" class="w-7 h-7 flex items-center justify-center bg-white/20 backdrop-blur-md rounded-lg text-white hover:bg-white/40 transition-colors">
