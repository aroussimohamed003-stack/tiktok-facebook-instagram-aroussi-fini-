...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['like_post_id'])) {
        ...
        if ($check->num_rows == 0) {
            ...
            // Notify owner - ALLOW SELF NOTIFICATION
            $p_query = $conn->query("SELECT user_id FROM posts WHERE id = $post_id");
            if ($p_query && $p_row = $p_query->fetch_assoc()) {
                $recipient = $p_row['user_id'];
                $conn->query("INSERT INTO notifications (recipient_id, sender_id, type, post_id) VALUES ($recipient, $user_id, 'like', $post_id)");
            }
        } else {
            // Unlike
    ...
    if (isset($_POST['comment_content']) && isset($_POST['post_id'])) {
        ...
        if (!empty($content)) {
            ...
            if ($insert) {
                ...
                if ($p_query && $p_row = $p_query->fetch_assoc()) {
                     $recipient = $p_row['user_id'];
                     $conn->query("INSERT INTO notifications (recipient_id, sender_id, type, post_id) VALUES ($recipient, $user_id, 'comment', $post_id)");
                }
                    ...
                    <div id="sidebar-friends-list">
                        ...
                        ?>
                            <a href="profile.php?user_id=<?= $sf['id'] ?>" class="d-flex align-items-center mb-2 text-decoration-none" style="color: var(--text-color);">
                                <img src="<?= $sf_pic ?>" class="rounded-circle me-2" style="width:35px;height:35px;object-fit:cover;">
                ...
                <div class="post-footer">
                    <div class="">
                        ...
                        <span style="cursor: pointer;" onclick="showLikers(<?php echo $row['id']; ?>)">
                            <span class="like-count"><?php echo $row['likes_count']; ?></span> إعجاب
                        </span>
                        ...
                        <!-- Share Button -->
                        <span class="ms-3" style="cursor: pointer;" onclick="openShareModal(<?php echo $row['id']; ?>, 'post')">
