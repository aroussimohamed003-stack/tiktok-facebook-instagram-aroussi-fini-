...
if ($action == 'add_product') {
    ...
    // Insert Product
    $stmt = $con->prepare("INSERT INTO market_products (user_id, category_id, subcategory_id, title, description, price, location) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("iiissss", $user_id, $cat_id, $subcat_id, $title, $desc, $price, $loc);
...
if ($action == 'fetch_products') {
    ...
    $min_price = $_GET['min_p'] ?? 0;
    $max_price = $_GET['max_p'] ?? 9999999;
    $subcat_id = $_GET['subcat'] ?? 0;
